...
namespace FluentMigrator.Builders.IfDatabase
{
    ...
    public class IfDatabaseExpressionRoot : IIfDatabaseExpressionRoot
    {
        ...
        /// <summary>
        /// Constructs a new instance of a <see cref="IfDatabaseExpressionRoot"/> that will only add expressions to the provided <paramref name="context"/> if <paramref name="databaseType"/> matches the migration processor
        /// </summary>
        ...
        public IfDatabaseExpressionRoot(IMigrationContext context, bool conditionValue, params string[] databaseType)
        {
            ...
            _context = DatabaseTypeApplies(context, databaseType) == conditionValue ? context : new MigrationContext(new MigrationConventions(), new NullIfDatabaseProcessor(), context.MigrationAssembly, context.ApplicationContext, "");
        }
